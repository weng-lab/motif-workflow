...
fun WorkflowBuilder.aggregateTask(type: String, i: Publisher<AggregateInput>, signalName: (signalBigWig: File) -> String): Flux<AggregateOutput> = this.task("aggregate-${type}", i) {
    ...
    val unzippedFile = OutputFile("${input.peaksBedGz.filenameNoExt()}.bed")
    ...
    val outputFileProximal = OutputFile("${bedPrefix}_${finalSignalPrefix}.proximal.aggregate.tsv")

    val intermediateFileDistal = OutputFile("${bedPrefix}d_${signalPrefix}.aggregate.tsv")
    val outputFileDistal = OutputFile("${bedPrefix}_${finalSignalPrefix}.distal.aggregate.tsv")
    ...
    command =
        """
        zcat ${input.peaksBedGz.dockerPath} > ${unzippedFile.dockerPath}
        java -jar /app/aggregate.jar \
            --peaks=${unzippedFile.dockerPath} \
