...
fun <T> WorkflowBuilder.splitPeaksTask(type: String, i: Publisher<SplitPeaksInput<T>>): Flux<SplitPeaksOutput<T>> = this.task("split-peaks-${type}", i) {
    ...
    output = SplitPeaksOutput(input.assembly, input.meta, input.peaksBedGz, proximalPeaks, distalPeaks)
    command =
        """
        set -e
        cat ${input.gencodeGTF.dockerPath} \
            ...
            | sort -k1,1 -k2,2n \
            > tss.bed
        zcat ${input.peaksBedGz.dockerPath} | sort -k1,1 -k2,2n > ${unzippedBed.dockerPath}
